...
export const updateOrderStatus = async (req, res) => {
  try {
    ...
    if (!isMasterAdmin) {
      if (["Packed", "Dispatched"].includes(status) && !isFranchise) {
        return handleResponse(
          res,
          403,
          "Only franchise can update to Packed/Dispatched",
        );
      }
      if (status === "Delivered" && !isDelivery && !isFranchise) {
        return handleResponse(
          res,
          403,
          "Only delivery partner can update to Delivered",
        );
      }
    ...
    // Auto-assign franchise on first franchise action (Packed/Dispatched)
    if (
      isFranchise &&
      ["Packed", "Dispatched"].includes(status) &&
      !order.franchiseId
    ) {
