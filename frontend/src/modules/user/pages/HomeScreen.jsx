...
export default function HomeScreen() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                // Fetch individually to prevent one failing call from blocking others
                api.get('/catalog/categories').then(res => {
                    if (res.data.success) setCategories(res.data.results)
                }).catch(err => console.error('Categories fetch failed:', err));

                api.get('/products').then(res => {
                    if (res.data.success) setProducts(res.data.results)
